...
import { Award, BookOpen, Calendar, GraduationCap, Newspaper, Sparkles, Users } from "lucide-react";
...
function PlaceholderCards() {
    ...
    return (_jsx(motion.div, { initial: { opacity: 0, y: 18 }, whileInView: { opacity: 1, y: 0 }, viewport: { once: true }, className: "grid grid-cols-2 gap-5 sm:grid-cols-3 lg:min-h-[430px] lg:grid-cols-6 lg:grid-rows-6 lg:items-stretch", children: cards.map((layout, index) => (_jsx(motion.div, { whileHover: { y: -6, boxShadow: "0 18px 40px rgba(23,55,94,0.22)" }, className: `min-h-[110px] rounded-btn bg-[#17375E] shadow-card transition ${layout}` }, index))) }));
}
