...
announcementRouter.get("/", attachUserOptional, [
    ...
    try {
        ...
        if (isAdmin && req.query.active === "true")
...
announcementRouter.post("/", verifyToken, [body("title").isString().notEmpty(), body("body").optional().isString()], async (req, res, next) => {
    try {
        const errors = validationResult(req);
        if (!errors.isEmpty())
            return res.status(400).json({ errors: errors.array() });
...
announcementRouter.put("/:id", verifyToken, async (req, res, next) => {
    try {
...
announcementRouter.delete("/:id", verifyToken, async (req, res, next) => {
    try {
