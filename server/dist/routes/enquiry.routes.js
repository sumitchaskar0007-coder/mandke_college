...
enquiryRouter.post("/", [
    ...
    try {
        ...
        if (adminTo) {
            ...
            await sendMail(adminTo, subjectLine, `<p><strong>Name:</strong> ${req.body.name}</p>
...
enquiryRouter.patch("/:id", verifyToken, async (req, res, next) => {
    try {
        const { isRead, repliedAt } = req.body;
        const update = {};
        if (typeof isRead === "boolean")
            update.isRead = isRead;
        if (typeof repliedAt === "string")
            update.repliedAt = new Date(repliedAt);
