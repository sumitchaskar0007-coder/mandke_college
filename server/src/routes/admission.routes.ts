...
admissionRouter.post(
  ...
  async (req: Request, res: Response, next: NextFunction) => {
    try {
      ...
      const studentEmail = req.body.email as string;
      await sendMail(
        studentEmail,
        "Admission application received — Mandke College",
        `<p>Dear ${req.body.fullName},</p><p>Thank you for applying to Mandke College. Our admissions office will reach out shortly.</p>`
      );
      ...
      if (adminTo) {
        await sendMail(
          adminTo,
          `[Admission] Application from ${req.body.fullName}`,
...
admissionRouter.patch("/:id", verifyToken, async (req: Request, res: Response, next: NextFunction) => {
  try {
