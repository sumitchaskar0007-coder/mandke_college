...
blogRouter.get("/slug/:slug", attachUserOptional, async (req: AuthRequest, res: Response, next: NextFunction) => {
  try {
    const post = await Blog.findOne({ slug: req.params.slug });
    if (!post) return res.status(404).json({ message: "Not found" });
    if (!post.isPublished && !req.user) return res.status(404).json({ message: "Not found" });
...
blogRouter.post(
  ...
  async (req: AuthRequest, res: Response, next: NextFunction) => {
    try {
      ...
      let slug =
        typeof (req.body as { slug?: string }).slug === "string" && (req.body as { slug?: string }).slug!.length > 0
          ? String((req.body as { slug?: string }).slug)
          : title
              .toLowerCase()
              .replace(/[^a-z0-9\s-]/g, "")
              .trim()
...
blogRouter.put("/:id", verifyToken, async (req: AuthRequest, res: Response, next: NextFunction) => {
  try {
...
blogRouter.delete("/:id", verifyToken, async (req: AuthRequest, res: Response, next: NextFunction) => {
  try {
...
blogRouter.patch("/:id/publish", verifyToken, async (req: AuthRequest, res: Response, next: NextFunction) => {
  try {
    const current = await Blog.findById(req.params.id);
    if (!current) return res.status(404).json({ message: "Not found" });
    const explicit = typeof (req.body as { isPublished?: boolean }).isPublished === "boolean"
      ? (req.body as { isPublished?: boolean }).isPublished
      : undefined;
