...
galleryRouter.get("/categories", (_req: Request, res: Response) => {
  res.json(GALLERY_CATEGORIES);
});

galleryRouter.get("/albums", async (_req: Request, res: Response, next: NextFunction) => {
  try {
...
galleryRouter.post(
  "/albums",
  verifyToken,
  [body("title").isString().trim().notEmpty(), body("slug").optional().isString(), body("category").optional().isString()],
  async (req: Request, res: Response, next: NextFunction) => {
    try {
      const errors = validationResult(req);
      if (!errors.isEmpty()) return res.status(400).json({ errors: errors.array() });
...
galleryRouter.post(
  "/",
  verifyToken,
  uploadImagesMemory.array("images", 24),
  async (req: Request, res: Response, next: NextFunction) => {
    try {
      ...
      if (!files?.length) return res.status(400).json({ message: "images required" });
...
galleryRouter.delete("/:id", verifyToken, async (req: Request, res: Response, next: NextFunction) => {
  try {
